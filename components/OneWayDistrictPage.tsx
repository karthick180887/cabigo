...
import { getServiceSubdistrictsByDistrictSlug } from "@/lib/data/service-areas";
...
export default function OneWayDistrictPage({ slug }: Props) {
    ...
    const serviceSubdistricts = getServiceSubdistrictsByDistrictSlug(district.slug);
    const pickupAreas = serviceSubdistricts.slice(0, 16);
                        ...
                        <div className={styles.mainContent}>
                            ...
                            {pickupAreas.length > 0 && (
                                <div className={styles.contentBlock}>
                                    ...
                                    <p>
                                        We offer one way drop taxi pickup across major taluks in{" "}
                                        {district.name}. Select your area to start your booking.
                                    </p>
                            ...
                            <div className={styles.vehicleSection}>
                                        ...
                                        <tbody>
                                            {districtVehicles.map(v => (
                                                <tr key={v.id}>
                                                    ...
                                                    <td>
                                                        {v.oneWayRatePerKm ? `₹${v.oneWayRatePerKm}/km` : "Ask for Quote"}
                                                        {v.driverBeta ? (
                                                            <div style={{ fontSize: "0.85rem", color: "var(--color-text-muted)" }}>
                                                                Driver bata ₹{v.driverBeta}
                                                            </div>
                                                        ) : null}
                                                    </td>
                                                    <td>{v.capacity}</td>
                                ...
                                <p style={{ marginTop: "0.75rem", fontSize: "0.9rem", color: "var(--color-text-muted)" }}>
                                    Toll fees, inter-state permit, waiting charges, parking charges, pet charges, and hill
                                    station charges (if any) are extra.
                                </p>
                            </div>
                            ...
                            <div className={styles.faqSection}>
                                ...
                                <div className={styles.faqList}>
                                    ...
                                    <details className={styles.faqItem}>
                                        ...
                                        <p>
                                            There are no hidden charges. Toll, parking, state permit, and driver bata are
                                            billed separately and shared upfront before confirmation.
                                        </p>
                                    </details>
                                </div>
                            </div>
