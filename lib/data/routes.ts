...
const ONE_WAY_RATES_PER_KM = {
    sedan: vehicles.find((vehicle) => vehicle.id === "sedan")?.oneWayRatePerKm ?? 13,
    suv: vehicles.find((vehicle) => vehicle.id === "suv")?.oneWayRatePerKm ?? 18,
    premium: vehicles.find((vehicle) => vehicle.id === "innova-crysta")?.oneWayRatePerKm ?? 25,
};

function getRoutePricing(distanceKm: number) {
    return {
        sedan: Math.round(distanceKm * ONE_WAY_RATES_PER_KM.sedan),
        suv: Math.round(distanceKm * ONE_WAY_RATES_PER_KM.suv),
        premium: Math.round(distanceKm * ONE_WAY_RATES_PER_KM.premium),
    };
}
